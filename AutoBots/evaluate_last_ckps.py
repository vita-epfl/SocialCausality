...
class Evaluator:
    def __init__(self, args, model_config, model_dirname):
        ...
        self.start_step = args.save_step_start
        self.end_step = args.save_step_end
    ...
    def initialize_model(self):
        ...
        model_dicts = torch.load(self.args.models_path + 'models_step_' + str(self.start_step) + '.pth', map_location=self.device)
    ...
    def load_model_steps(self, steps):
        model_dicts = torch.load(self.args.models_path + 'models_step_' + str(steps) + '.pth', map_location=self.device)
        self.autobot_model.load_state_dict(model_dicts["AutoBot"])
        self.autobot_model.eval()
    ...
    def autobotego_evaluate(self):
        ADE_ls = []
        FDE_ls = []
        for step in range(self.start_step, self.end_step, self.eval_interval):
            self.load_model_steps(steps=step)
            print('step:', step)
            with torch.no_grad():
                val_ade_losses = []
                val_fde_losses = []
                ...
                ADE_ls.append(val_minade_c[0])
                FDE_ls.append(val_minfde_1[0])
        
        # calculate mean and std
        ADE_ls = np.array(ADE_ls)
        FDE_ls = np.array(FDE_ls)

        ADE_mean = np.mean(ADE_ls, axis=0)
        FDE_mean = np.mean(FDE_ls, axis=0)
        ADE_std = np.std(ADE_ls, axis=0)
        FDE_std = np.std(FDE_ls, axis=0)
        print('ADE mean:', ADE_mean, 'ADE std:', ADE_std)
