...
for weight in 0.25 0.5 0.75; do
    for low_data in 0.05 0.1 0.2 0.3 0.5 0.8 1.0; do
        python train.py \
            --exp-id s2r_baseline_hotel_${low_data}_pw_${weight} \
            --seed 1 \
        ...
        python evaluate_last_ckps.py --dataset-path ./datasets/eth-ucy/processed_6/hotel/ \
            --models-path /scratch/izar/luan/results/s2r_baseline/Autobot_ego_NScene:-1_regType:None_s2r_baseline_hotel_${low_data}_pw_${weight}_s1/ \
            --batch-size 64 --save_step_start 10000 --save_step_end 20000 --output_tag s2r_baseline_hotel_${low_data}_pw_${weight}
    done
done