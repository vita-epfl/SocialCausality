...
for batch in 64 8 16 32 128 256; do
    for low_data in 0.05 0.1 0.2 0.3 0.5 0.8 1.0; do
        python train.py \
            --exp-id s2r_pred_cl_zara1_$low_data \
            --seed 1 \
            --dataset s2r_pred_cl \
        ...
        python evaluate_last_ckps.py --dataset-path ./datasets/eth-ucy/processed_6/zara1/ \
            --models-path /scratch/izar/luan/results/s2r_pred_cl/Autobot_ego_NScene:-1_regType:None_CW:${weight}_CB:${batch}_s2r_pred_cl_zara1_${low_data}_s1/ \
            --batch-size 64 --save_step_start 10000 --save_step_end 15000 --output_tag s2r_pred_cl_zara1_w_${weight}_b_${batch}_p_$low_data
    done
done
