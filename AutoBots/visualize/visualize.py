...
def plot_Autobot_datasize_effect_curve(show_percentage=False):
    ...
    if os.path.isfile("datasize_effect_curve_datas.pkl"):
        ...
        for exp in os.listdir("/scratch/izar/arahimi/autobots_causality/results/synth/"):
            if f"_v2_{cutoff}_ft" in exp and "contrastive_" in exp:
                weight = int(exp.split(':')[-1].replace(f"_v2_{cutoff}_ft_s1", ""))
                evaluation_metrics[f"contrastive_{weight}"] = {"ADE": [], "ARS": [], "HNC": []}
            elif f"_v2_{cutoff}_ft_encoder_only" in exp and "consistency_" in exp:
                weight = int(exp.split(':')[-1].replace(f"_v2_{cutoff}_ft_encoder_only_s1", ""))
        ...
        for datasize in datasizes:
            for saving in evaluation_metrics.keys():
                ...
                if saving == "normal":
                    log_directory = "/scratch/izar/arahimi/autobots_causality/results/synth/Autobot_ego_NScene:{}_regType:None_v2_{}_bl_s1/tb_files".format(datasize, cutoff)
                else:
                    reg, weight = saving.split('_')
                    if reg == "contrastive":
                        log_directory = "/scratch/izar/arahimi/autobots_causality/results/synth/Autobot_ego_NScene:{}_regType:{}_CW:{}_v2_{}_ft_s1/tb_files".format(datasize, reg, weight, cutoff)
                    else:
                        log_directory = "/scratch/izar/arahimi/autobots_causality/results/synth/Autobot_ego_NScene:{}_regType:{}_CW:{}_v2_{}_ft_encoder_only_s1/tb_files".format(
                            datasize, reg, weight, cutoff)
                ...
                try:
                    ...
                    print("FileNotFoundError", log_file)
                    continue
    ...
    for metric in ["ARS", "HNC", "ADE"]:
        ...
        for key, value in evaluation_metrics.items():
            ...
            if len(data) > 0 and ("consistency" in key or "normal" in key or "contrastive_1000" in key):
                sns.lineplot(
                    x=datasizes,
                    y=data,
                    # c=color,
