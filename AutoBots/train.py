...
class Trainer:
    ...
    def initialize_dataloaders(self):
        ...
        if self.args.dataset == "synth":
            ...
            self.train_loader_sim = torch.utils.data.DataLoader(
                train_dset_sim, batch_size=args.batch_size_cl, shuffle=True, num_workers=12, drop_last=False,
                pin_memory=False, collate_fn=my_collate_fn
            )
    ...
    def autobotego_train(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.num_epochs):
            ...
            for i, data in enumerate(self.train_loader):
                ...
                elif self.args.dataset == "s2r": # sim2real
                ...
                if self.args.dataset == "synth" and self.args.reg_type == "consistency":
                    ...
                    loss_real = nll_loss + adefde_loss + kl_loss
                    (loss_real + self.args.s2r_causal * contrastive_loss + self.args.s2r_predict * simulation_loss).backward()
                else:
                ...
                if self.args.dataset == "synth" and self.args.reg_type == "consistency":
                    ...
                    self.writer.add_scalar("Loss/contrastive", contrastive_loss.item(), steps)
                    self.writer.add_scalar("Loss/simulation", simulation_loss.item(), steps)
                ...
                if steps >= args.save_step_start and steps < args.save_step_end:
                    if steps % self.args.save_every_ckp == 0:
                        # save ckps every step
                        self.save_model_step(steps)
                ...
                if i % 10 == 0:
                    if self.args.dataset == "synth" and self.args.reg_type == "consistency":
                        ...
                        print(i, "/", len(self.train_loader_real.dataset) // self.args.batch_size,
                              ...
                              "Post Entropy", round(post_entropy, 2), "ADE+FDE loss", round(adefde_loss.item(), 2),
                              "Contrastive loss", round(contrastive_loss.item(), 2),
                              "Simulation loss", round(simulation_loss.item(), 2))
                    else:
                        print(i, "/", len(self.train_loader.dataset) // self.args.batch_size,
                              "NLL loss", round(nll_loss.item(), 2), "KL loss", round(kl_loss.item(), 2),
                              "Prior Entropy", round(torch.mean(D.Categorical(mode_probs).entropy()).item(), 2),
            ...
            # temporarily turn off early stop
    ...
    def autobotego_evaluate_step(self, step):
        ...
        with torch.no_grad():
            ...
            if self.args.evaluate_causal:
                ...
                self.writer.add_scalar("metrics/Val ARS", val_ARS, step)
    ...
    def save_init_model(self):
        torch.save(
            ...
            os.path.join(self.results_dirname, "models_%d.pth" % -1),
        )

    def save_model_step(self, step=None, minade_k=None, minfde_k=None):
        if step is None:
            ...
            torch.save(
                ...
                os.path.join(self.results_dirname, "models_step_%d.pth" % step),
            )
